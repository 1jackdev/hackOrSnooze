...
class User {
  ...
  static async getFavorites(user) {
    // query the user endpoint (no auth required)
    const response = await axios.get(
      `${BASE_URL}/users/${user.username}/?token=${user.loginToken}`,
      {
        token: user.loginToken,
        user: user.username,
      }
  ...
  static async removeFavorite(user, storyId) {
    const response = await axios.delete(
      `${BASE_URL}/users/${user.username}/favorites/${storyId}?token=${user.loginToken}`,
      {
        token: user.loginToken,
        username: user.username,
        storyId: storyId,
  ...
  static async getMyStories(user) {
    ...
    // // build an instance of our own class using the new array of stories
    const myStoriesList = new StoryList(stories);
    return myStoriesList;
  }

  static async removeStory(user, storyId) {
    const response = await axios.delete(
      `${BASE_URL}/stories/${storyId}?token=${user.loginToken}`,
      {
        token: user.loginToken,
        username: user.username,
        storyId: storyId,
